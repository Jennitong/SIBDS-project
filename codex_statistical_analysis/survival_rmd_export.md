...
Do **not** pass `"skip_html"` — it would suppress `<a>` tags and inline HTML entities. Do **not** pass `"toc"` either — the `markdown` package doesn't resolve it against this document's headers, so it leaves a raw, unstyled `<toc id="...">...</toc>` list box at the top of the page instead of a real table of contents.
