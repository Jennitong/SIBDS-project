...
## Step 9 — R Markdown / HTML Export (if requested)

**This step always runs immediately after the output above (Step 8 and the Output Specification), for every detail level (brief, moderate, detailed) and regardless of whether Step 5's pairwise table was shown. Do not skip it and do not let the Edge Cases, Table Formatting, or Notes reference sections below stand in for it.**
